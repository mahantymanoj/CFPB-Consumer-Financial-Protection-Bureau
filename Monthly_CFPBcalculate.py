...
import datetime, urllib3, calendar
import pandas as pd
...
csv_path = r'D:\Data\ESG\CFPB\Complaint-DB\CSV\Monthly\\' # Read from Complaint DB
save_path = r'D:\Task\Log\Basu Sir\ESG\CFPB'+'\\' + datetime.datetime.today().strftime('%Y%m%d')
...
save_path +=  '\\CFPB_' + str(datetime.today().strftime('%Y%m')) + '.xlsx'
...
for j in range(0,5):
    ...
    for month in monthList:
        if month<10:
            ...
            month = str(month)
        dateList.append(str(year)+str(month)+'.csv')

    # rollover dates
    year = year-1
    month = today.month
    for mth in range(month,12+1):
        dateList.append(str(year)+str(mth)+'.csv')
